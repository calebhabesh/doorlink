...
export default function SystemHealth() {
  ...
  return (
    <div className="w-full max-w-[1200px] mx-auto flex flex-col h-full flex-1">
      <div className="mb-8 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-black tracking-tight text-white">Infrastructure & Device Health</h2>
            <p className="text-sm text-zinc-400 font-medium">Measured gateway components and last reported physical doorbell telemetry</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="md:col-span-2 lg:col-span-4 bg-zinc-950 border border-zinc-800 p-6 rounded-3xl shadow-lg">
          <div className="flex justify-between items-start mb-5">
            <Cpu className="w-5 h-5 text-emerald-400" />
            <StatusBadge status={health?.device.status} />
          </div>
          <h3 className="text-zinc-500 text-sm font-bold uppercase tracking-widest mb-3">Doorbell Device</h3>
          ...
          <p className="text-xs text-zinc-500 mt-5 font-mono">
            Device contact and battery voltage are reported when an event wakes the doorbell. Charge percentage is an estimate from voltage, not a fuel-gauge measurement; charging state and battery temperature are not available.
          </p>
        </div>
      </div>
    </div>
...
function HealthCard({ icon, title, status, value, detail }: { icon: React.ReactNode; title: string; status?: string; value: string; detail: string }) {
  return (
    <div className="bg-zinc-950 border border-zinc-800 p-6 rounded-3xl shadow-lg">
      <div className="flex justify-between items-start mb-6">{icon}<StatusBadge status={status} /></div>
      <h3 className="text-zinc-500 text-sm font-bold uppercase tracking-widest mb-1">{title}</h3>
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-3">
      <span className="text-zinc-500 block text-xs uppercase tracking-wider mb-1">{label}</span>
